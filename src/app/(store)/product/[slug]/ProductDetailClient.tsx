...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  const getSelectedVariantName = () => {
    if (!selectedVariant || !product.variants) return undefined;
    const v = product.variants.find((v: any) => v.id === selectedVariant);
    return v?.name;
  };

  const handleAddToCart = async () => {
    await addItem(product.id, quantity, selectedVariant || undefined, productData, getSelectedVariantName());
    setAddedToCart(true);
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`rounded-xl border p-3 transition-colors ${
                  mounted && has(product.id) ? 'border-red-300 bg-red-50 text-red-500' : 'border-gray-200 hover:border-red-300 hover:text-red-500'
                }`}
              >
                <Heart size={20} className={mounted && has(product.id) ? 'fill-red-500' : ''} />
              </button>
            </div>
